...
export async function fetchRecipeIdBySlug(slug: string): Promise<string | null> {
    ...
    const { data, error } = await supabase.from("recipes").select("id").eq("site_slug", slug).maybeSingle();
...
export async function fetchBaseDetail(slug: string): Promise<DetailBase | null> {
    ...
    // 1) RPC primeiro
    const { data: rpcData, error: rpcErr } = await supabase.rpc("get_public_recipe_detail", { p_slug: slug });
    if (rpcErr) console.warn("[get_public_recipe_detail]", rpcErr);
    const fromRpc = (rpcData && (rpcData as any[])[0]) as Partial<DetailBase> | undefined;
    ...
    if (!fromRpc?.description || !fromRpc?.ingredients?.length) {
        const { data: row } = await supabase
            .from("recipes")
            .select("site_slug,name,short_description,description,cover_url,youtube_url,preferir_link_youtube,prep_time_minutes,difficulty_slug,publicado_at")
            .eq("site_slug", slug)
            .maybeSingle();
    ...
    if (!fromRpc?.ingredients?.length && recipeId) {
    ...
    return {
        site_slug: fromRpc?.site_slug ?? fallbackRow.site_slug,
        name: fromRpc?.name ?? fallbackRow.name,
        short_description: fromRpc?.short_description ?? fallbackRow.short_description ?? null,
        description: fromRpc?.description ?? fallbackRow?.description ?? null,
        cover_url: fromRpc?.cover_url ?? fallbackRow?.cover_url ?? null,
        youtube_url: fromRpc?.youtube_url ?? fallbackRow?.youtube_url ?? null,
        preferir_link_youtube: fromRpc?.preferir_link_youtube ?? fallbackRow?.preferir_link_youtube ?? false,
        prep_time_minutes: fromRpc?.prep_time_minutes ?? fallbackRow?.prep_time_minutes ?? null,
        difficulty_slug: (fromRpc?.difficulty_slug ?? fallbackRow?.difficulty_slug ?? null) as any,
        publicado_at: fromRpc?.publicado_at ?? fallbackRow?.publicado_at ?? null,
        ingredients: (fromRpc?.ingredients as Ingredient[] | undefined) ?? fallbackIngredients,
    };
}
...
export async function fetchTips(recipeId: string): Promise<RecipeTip[]> {
    ...
    const { data, error } = await supabase
        .from("recipe_tips")
        .select("id, type, title, text, position, instruction_id, recipe_ingredient_id, created_at, updated_at")
        .eq("recipe_id", recipeId)
        .order("position", { ascending: true });
...
export async function fetchTaxonomies(recipeId: string): Promise<RecipeTaxonomies> {
    ...
    return {
        ...
        cuisines: d.cuisines ?? [],
        diet_labels: d.diet_labels ?? d.diet ?? [],
        techniques: d.techniques ?? [],
        occasions: d.occasions ?? [],
    };
}

export async function fetchTechniquesMap(): Promise<Record<string, { slug: string; label: string }>> {
    const supabase = await createSupabaseRSCClient();
    const { data, error } = await supabase.from("techniques").select("id, slug, label_ptbr");
    if (error) {
    ...
    (data ?? []).forEach((t) => {
        map[t.id] = { slug: (t as any).slug, label: (t as any).label_ptbr };
    });
    return map;
}
...
export function groupTipsByIngredient(tips: RecipeTip[]): Record<string, RecipeTip[]> {
    ...
    for (const t of tips) {
        ...
        byIng[t.recipe_ingredient_id] ??= [];
        byIng[t.recipe_ingredient_id].push(t);
    }
